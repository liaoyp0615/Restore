...
def load_data(data, max_n):
    ...
    d.close()
    df = shuffle(df)  #very important, add step of shuffling.
...
class Generator(nn.Module):
    def __init__(self, z_dim: int, hidden_size: int, data_dim: int) -> None:
        ...
        self.l3 = nn.Sequential( 
                  ...
                  #nn.ReLU()  # no using activation function 
                  )
...
if __name__ == '__main__':
    ...
    #g_hidden_size: int = 15     # set g_hidden_size int=20~
    g_data_dim: int = 1
    z_dim: int = 3
