...
"""
ops contains operations that we will use.
"""
...
def scale(x, v):
    return Lambda(lambda _: _ / v)(x) #将任意表达式封装成Layer对象   ？？？？
...
def inpainting_attention(primary, carryover, constant=-10):
    ...
    x = concatenate([primary, carryover], axis=-1) #串联起来
...
def energy_error(requested_energy, recieved_energy):
    ...
    over_energized = K.cast(difference > 0., K.floatx()) # x , dtype   将张量转换到不同的dtype并返回 ？？？？
...
def minibatch_discriminator(x):
    ...
    diffs = K.expand_dims(x, 3) - \
        K.expand_dims(K.permute_dimensions(x, [1, 2, 0]), 0) # 统一加一维，加3的减去加0的换轴的
    l1_norm = K.sum(K.abs(diffs), axis=2) # 计算某一指定轴的和
